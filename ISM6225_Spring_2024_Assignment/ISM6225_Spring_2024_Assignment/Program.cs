...
namespace ISM6225_Spring_2024_Assignment
{
    class Program
    {
        ...
        public static IList<int> MoveZeroes(int[] nums)
        {
            try
            {
                ...
                for (int j = 0; j < nums.Length; j++)
                {
                    if (nums[j] != 0)
                    {
                        // Swap non-zero element with the current position i
                        int temp = nums[i];
                        nums[i] = nums[j];
                        nums[j] = temp;
                        i++;
        ...
        public static int BinaryToDecimal(int binary)
        {
            try
            {
                ...
                while (binary > 0)
                {
                    ...
                    decimalValue += lastDigit * baseValue; // Update decimal value
        ...
        */
        ...
        public static int MaximumGap(int[] nums)
        {
            try
            {
                ...
                // Initialize the minimum and maximum values for each bucket
        ...
        */
        ...
        public static int LargestPerimeter(int[] nums)
        {
            try
            {
                ...
                // Start from the largest side and check if it forms a triangle with the two smaller sides
                for (int i = nums.Length - 1; i >= 2; i--)
                {
                    if (nums[i - 2] + nums[i - 1] > nums[i])
                    {
                        return nums[i - 2] + nums[i - 1] + nums[i]; // Found a triangle with non-zero area
                    }
                }

                return 0; // No triangle with non-zero area found
            }
            catch (Exception)
            {
                throw;
